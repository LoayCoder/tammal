...
import { useAwardThemeName, useEmployeeNames } from '../hooks/useRecognitionQueries';
...
export function NominationDetailDialog({
  ...
  // Fetch theme name
  const { data: themeName } = useAwardThemeName(nomination?.theme_id);

  // Resolve endorser names
  const endorserIds = endorsements.map(e => e.endorser_id);
  const { data: endorserMap = {} } = useEmployeeNames(endorserIds);
