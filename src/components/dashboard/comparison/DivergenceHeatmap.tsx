...
export function DivergenceHeatmap({ data, isLoading }: Props) {
  ...
  if (data.length === 0) {
    return (
      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-base">{t('synthesis.heatmapTitle')}</CardTitle></CardHeader>
        <CardContent><p className="text-sm text-muted-foreground text-center py-6">{t('synthesis.insufficientData')}</p></CardContent>
      </Card>
    );
  }
