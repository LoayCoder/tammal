...
import { Landmark, House, BriefcaseBusiness, ChevronRight, Clock4, Hourglass, CircleCheck, X } from 'lucide-react';
...
function PrayerCountdownBadge({ prayerTime }: { prayerTime: string }) {
  ...
  return (
    <span className="flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-600 border border-amber-500/30">
      <Hourglass className="h-3 w-3" strokeWidth={ICON_STROKE} />
...
export function DashboardPrayerWidget() {
  ...
  const activePrayer = useMemo(() => {
    ...
    // CircleCheck 5 obligatory prayers first
    ...
    // CircleCheck Witr (active when 22:00+ or before Fajr and not logged)
    ...
    // CircleCheck if Witr is still pending (before its window)
          ...
          <div className="rounded-xl bg-muted/50 p-3 space-y-2.5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground flex items-center gap-1">
                  <Clock4 className="h-3 w-3" strokeWidth={ICON_STROKE} />
                  {activePrayer === 'Witr'
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              <Button size="sm" variant="outline" onClick={() => handleLog('completed_work')} disabled={logPrayer.isPending} className="gap-1 h-7 text-xs">
                <BriefcaseBusiness className="h-3 w-3" strokeWidth={ICON_STROKE} /> {t('spiritual.prayer.work')}
              </Button>
            </div>
            ...
            {hasActiveRawatib && (
              <div className="flex flex-wrap gap-1.5">
                {activeRawatib.before && (
                  ...
                  >
                    📿
                    {todayCompleted.has(`rawatib_${activePrayer.toLowerCase()}_before`) && <CircleCheck className="h-3 w-3" strokeWidth={ICON_STROKE} />}
                ...
                {activeRawatib.after && (
                  ...
                  >
                    📿
                    {todayCompleted.has(`rawatib_${activePrayer.toLowerCase()}_after`) && <CircleCheck className="h-3 w-3" strokeWidth={ICON_STROKE} />}
