...
export const LandingShowcase = () => {
  ...
  return (
    <section id="showcase" className="py-24 lg:py-32 bg-gray-50">
          ...
          <div key={item.titleKey} className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <AnimatedSection delay={0}>
              <div className={i % 2 === 1 ? "lg:order-last" : ""}>
                <p className="text-[11px] uppercase tracking-[0.2em] text-blue-600/60 mb-3 relative inline-block">
                  0{i + 1}
                  <span className="absolute -inset-2 bg-blue-100/50 rounded-full blur-lg -z-10" />
                </p>
                <h3 className="text-2xl lg:text-3xl font-bold text-gray-900 tracking-tight mb-4">{t(item.titleKey)}</h3>
                <p className="text-sm text-gray-500 leading-relaxed max-w-md">{t(item.descKey)}</p>
              </div>
            </AnimatedSection>
