import type { CategoryScore, SubcategoryScore, RiskTrendPoint } from '@/hooks/useOrgAnalytics';
...
function computeDeclineRate(trend: CategoryTrendPoint[]): number {
  ...
  return Math.max(0, avgFirst - avgSecond); // positive = declining
}
...
export function computeHealthScore(avgMood: number, participationRate: number, riskPercentage: number): number {
  // Weighted: mood 40%, participation 30%, inverse risk 30%
