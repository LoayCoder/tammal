...
import {
  Plus, ListChecks, CalendarDays, BarChart3, Flame, Star,
...
export default function PersonalCommandCenter() {
  ...
  const { tasks, isLoading: tasksLoading, createTask, updateTask, deleteTask, isCreating, isUpdating } = useUnifiedTasks(employee?.id);
  ...
  const stats = useMemo(() => {
    ...
    const todayTasks = active; // Show all active for "My Day"
    const scheduledMinutes = todayTasks.reduce((sum, t) => sum + (t.estimated_minutes ?? 0), 0);
    return { active, done, overdue, todayTasks, scheduledMinutes, total: tasks.length };
  }, [tasks]);
        ...
        <Card className="glass-stat border-0">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-xs">{t('commandCenter.activeTasks')}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="glass-stat border-0">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-xl font-bold">{stats.done.length}</div>
              <p className="text-muted-foreground text-xs">{t('commandCenter.completed')}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="glass-stat border-0">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-xl font-bold">{stats.overdue.length}</div>
              <p className="text-muted-foreground text-xs">{t('commandCenter.overdue')}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="glass-stat border-0">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-xs">{t('commandCenter.streak')}</p>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <Tabs value={tab} onValueChange={setTab}>
            <TabsList className="mb-4">
              <TabsTrigger value="today" className="gap-1.5 text-xs">
                <CalendarDays className="h-3.5 w-3.5" />{t('commandCenter.myDay')}
              </TabsTrigger>
              <TabsTrigger value="all" className="gap-1.5 text-xs">
                <ListChecks className="h-3.5 w-3.5" />{t('commandCenter.allTasks')}
              </TabsTrigger>
              <TabsTrigger value="done" className="gap-1.5 text-xs">
                <CheckCircle2 className="h-3.5 w-3.5" />{t('commandCenter.completedTab')}
              </TabsTrigger>
            </TabsList>
            <TabsContent value="today">
              {tasksLoading ? <Skeleton className="h-40" /> : (
                <UnifiedTaskList tasks={stats.active} onEdit={handleEdit} onDelete={deleteTask} onToggle={handleToggle} />
              )}
            </TabsContent>
            <TabsContent value="all">
              {tasksLoading ? <Skeleton className="h-40" /> : (
                <UnifiedTaskList tasks={tasks} onEdit={handleEdit} onDelete={deleteTask} onToggle={handleToggle} />
              )}
            </TabsContent>
            <TabsContent value="done">
              {tasksLoading ? <Skeleton className="h-40" /> : (
                <UnifiedTaskList tasks={stats.done} onEdit={handleEdit} onDelete={deleteTask} onToggle={handleToggle} />
              )}
            </TabsContent>
          </Tabs>
        </CardContent>
