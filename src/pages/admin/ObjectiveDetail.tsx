...
import { Plus, ArrowRight, Pencil, Trash2, ChevronLeft, Rocket, FolderOpen } from 'lucide-react';
import {
...
export default function ObjectiveDetail() {
  ...
  const { objectives, isLoading: objLoading } = useObjectives();
  const { initiatives, isLoading: initLoading, createInitiative, updateInitiative, deleteInitiative, isCreating: initCreating, isUpdating: initUpdating, isDeleting: initDeleting } = useInitiatives(id);

  const objective = objectives.find(o => o.id === id);

  // Initiative + Action dialog state
  ...
  const { actions, createAction, updateAction, deleteAction, isCreating: actCreating, isUpdating: actUpdating, isDeleting: actDeleting } = useActions(expandedInitiative || undefined);
